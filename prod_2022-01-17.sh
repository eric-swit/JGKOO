cd $HOME/go/src/swit/swit-grpc-message-golang 
git tag release-v5.3.11

git push origin release-v5.3.11


cd $HOME/go/src/swit/swit-grpc-task-golang
git tag release-v5.3.8

git push origin release-v5.3.8


cd $HOME/go/src/swit/swit-apiv1
git tag release-v1.3.12

git push origin release-v1.3.12


cd $HOME/go/src/swit/swit-apiv5-golang
git tag  release-v1.3.7
git push origin  release-v1.3.7

cd $HOME/go/src/swit/swit-grpc-channel-golang
git tag release-v4.3.4

git push origin release-v4.3.4


cd $HOME/go/src/swit/swit-grpc-project-golang
git tag release-v4.3.3

git push origin release-v4.3.3


cd $HOME/go/src/swit/swit-grpc-contents-golang
git tag release-v1.3.11

git push origin release-v1.3.11


cd $HOME/go/src/swit/swit-grpc-asset-golang
git tag release-v1.3.5

git push origin release-v1.3.5


cd $HOME/go/src/swit/swit-gcs-file-golang
git tag release-v1.3.7

git push origin release-v1.3.7


cd $HOME/go/src/swit/swit-grpc-channel-golang
git tag release-v5.3.2

git push origin release-v5.3.2


cd $HOME/go/src/swit/swit-grpc-project-golang
git tag release-v5.3.2

git push origin release-v5.3.2


cd $HOME/go/src/swit/swit-openapi-golang
git tag release-v1.3.5

git push origin release-v1.3.5
 
 
